...
const removeDelimitersBetween = function (
  bottom: IInlineDelimiters,
  top: IInlineDelimiters
) {
  if (bottom.next !== top) {
    bottom.next = top;
    top.previous = bottom;
...
class InlineParser {
  ...
  delimiters: IInlineDelimiters | null = null; // used by handleDelim method

  brackets: IInlineBrackets | null = null;
  ...
  parseReference(s: string, refmap: InlineParser["refmap"]) {
    this.subject = s;
    this.pos = 0;
  ...
  processEmphasis(stack_bottom: IInlineDelimiters | null) {
    let opener: IInlineDelimiters | null;
    let closer: IInlineDelimiters | null;
    let old_closer: IInlineDelimiters | null;

    let opener_inl: MarkdownNode | null;
    let closer_inl: MarkdownNode | null;

    let tempstack: IInlineDelimiters | null;
    let use_delims: number;

    let tmp: MarkdownNode | null;
    let next: MarkdownNode | null;
    let opener_found: boolean;
    let openers_bottom: (IInlineDelimiters | null)[] = [];
    let openers_bottom_index!: number;
    let odd_match = false;
  ...
  removeDelimiter(delim: IInlineDelimiters) {
