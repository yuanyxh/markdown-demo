...
class DefinitionMap<D> {
  ...
  private readonly definitions = new Map<string, D>();
  ...
  public addAll(that: DefinitionMap<D>): void {
   ...
   */
  public putIfAbsent(label: string, definition: D): D {
   ...
   */
  public get(label: string): D | undefined {
    const normalizedLabel: string = Escaping.normalizeLabelContent(label);
