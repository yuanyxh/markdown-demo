class FencedCodeBlockParser extends AbstractBlockParser {
  ...
  private otherLines: stringBuilder | null = new stringBuilder();
  ...
  public static Factory = class Factory extends AbstractBlockParserFactory {
    public tryStart(
      ...
      let nextNonSpace: int = state.getNextNonSpaceIndex();
      let blockParser: FencedCodeBlockParser =
        FencedCodeBlockParser.checkOpener(
  ...
  private static checkOpener(
    ...
    let tildes: int = 0;
    let length: int = line.length();
    loop: for (let i: int = index; i < length; i++) {
  ...
  private tryClosing(line: java.lang.CharSequence | null, index: int): boolean {
    ...
    let after: int = Characters.skipSpaceTab(
      line,
      index + fences,
      line.length()
    );
