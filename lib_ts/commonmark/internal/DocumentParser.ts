...
export  class DocumentParser extends JavaObject implements ParserState {
    ...
    private static readonly  CORE_FACTORY_TYPES:  java.util.Set<java.lang.Class< Block>> | null = new  java.util.LinkedHashSet(java.util.List.of(
            BlockQuote.class,
            Heading.class,
    ...
    private static readonly  NODES_TO_CORE_FACTORIES:  java.util.Map<java.lang.Class< Block>, BlockParserFactory> | null;

     static {
        let  map: java.util.Map<java.lang.Class< Block>, BlockParserFactory> = new  java.util.HashMap();
        ...
        DocumentParser.NODES_TO_CORE_FACTORIES = java.util.Collections.unmodifiableMap(map);
    }
    ...
    private readonly  openBlockParsers:  java.util.List<DocumentParser.OpenBlockParser> | null = new  java.util.ArrayList();
    private readonly  allBlockParsers:  java.util.List<BlockParser> | null = new  java.util.ArrayList();

    public  constructor(blockParserFactories: java.util.List<BlockParserFactory>| null, inlineParserFactory: InlineParserFactory| null,
                          inlineContentParserFactories: java.util.List<InlineContentParserFactory>| null, delimiterProcessors: java.util.List<DelimiterProcessor>| null,
                          linkProcessors: java.util.List<LinkProcessor>| null, linkMarkers: java.util.Set<java.lang.Character>| null, includeSourceSpans: IncludeSourceSpans| null) {
        super();
this.blockParserFactories = blockParserFactories;
        ...
        this.activateBlockParser(new  DocumentParser.OpenBlockParser(this.documentBlockParser, 0));
    }

    public static  getDefaultBlockParserTypes():  java.util.Set<java.lang.Class< Block>> | null {
        return DocumentParser.CORE_FACTORY_TYPES;
    }

    public static  calculateBlockParserFactories(customBlockParserFactories: java.util.List<BlockParserFactory>| null, enabledBlockTypes: java.util.Set<java.lang.Class< Block>>| null):  java.util.List<BlockParserFactory> | null {
    ...
    public static  checkEnabledBlockTypes(enabledBlockTypes: java.util.Set<java.lang.Class< Block>>| null):  void {
        for (let enabledBlockType of enabledBlockTypes) {
            if (!DocumentParser.NODES_TO_CORE_FACTORIES.containsKey(enabledBlockType)) {
                throw new  java.lang.IllegalArgumentException("Can't enable block type " + enabledBlockType + ", possible options are: " + DocumentParser.NODES_TO_CORE_FACTORIES.keySet());
            }
        }
    }
     ...
     */
    public  parse(input: java.lang.String| null):  Document | null;

    public  parse(input: java.io.Reader| null):  Document | null;
public parse(...args: unknown[]):  Document | null {
		switch (args.length) {
			case 1: {
				const [input] = args as [java.lang.String];


        let  lineStart: int = 0;
        let  lineBreak: int;
        while ((lineBreak = Characters.findLineBreak(input, lineStart)) !== -1) {
            let  line: java.lang.String = input.substring(lineStart, lineBreak);
        ...
        if (!input.isEmpty() && (lineStart === 0 || lineStart < input.length())) {
            let  line: java.lang.String = input.substring(lineStart);
            this.parseLine(line, lineStart);
        }
     ...
     */
    private  parseLine(ln: java.lang.String| null, inputIndex: int):  void {
        ...
        for (let  i: int = 1; i < this.openBlockParsers.size(); i++) {
            let  openBlockParser: DocumentParser.OpenBlockParser = this.openBlockParsers.get(i);
        ...
        let  blockParser: BlockParser = this.openBlockParsers.get(matches - 1).blockParser;
        ...
        while (tryBlockStarts) {
            ...
            if (blockStart.isReplaceActiveBlockParser()) {
                let  replacedBlock: Block = this.prepareActiveBlockParserForReplacement();
    ...
    private  setLine(ln: java.lang.String| null, inputIndex: int):  void {
        this.lineIndex++;
        this.index = 0;
        this.column = 0;
        this.columnIsInTab = false;

        let  lineContent: java.lang.String = DocumentParser.prepareLine(ln);
...
default:
    ...
    private  addLine():  void {
        ...
        if (this.columnIsInTab) {
            ...
            let  rest: java.lang.CharSequence = this.line.getContent().subSequence(afterTab, this.line.getContent().length());
            let  spaces: int = Parsing.columnsToNextTabStop(this.column);
            let  sb: java.lang.StringBuilder = new  java.lang.StringBuilder(spaces + rest.length());
    ...
    private  addSourceSpans():  void {
        if (this.includeSourceSpans !== IncludeSourceSpans.NONE) {
            ...
            for (let  i: int = 1; i < this.openBlockParsers.size(); i++) {
                ...
                let  blockIndex: int = java.lang.Math.min(openBlockParser.sourceIndex, this.index);
    ...
    private  findBlockStart(blockParser: BlockParser| null):  BlockStartImpl | null {
        let  matchedBlockParser: MatchedBlockParser = new  DocumentParser.MatchedBlockParserImpl(blockParser);
    ...
    private  processInlines():  void {
        let  context  = new  InlineParserContextImpl(this.inlineContentParserFactories, this.delimiterProcessors, this.linkProcessors, this.linkMarkers, this.definitions);
     ...
     */
    private  addChild(openBlockParser: DocumentParser.OpenBlockParser| null):  void {
    ...
    private  activateBlockParser(openBlockParser: DocumentParser.OpenBlockParser| null):  void {
     ...
     */
    private static  prepareLine(line: java.lang.String| null):  java.lang.String | null {
        if (line.indexOf('\0') === -1) {
            return line;
        } else {
    ...
    public static MatchedBlockParserImpl =  class MatchedBlockParserImpl extends JavaObject implements MatchedBlockParser {
...
export namespace DocumentParser {
	export type MatchedBlockParserImpl = InstanceType<typeof DocumentParser.MatchedBlockParserImpl>;
	export type OpenBlockParser = InstanceType<typeof DocumentParser.OpenBlockParser>;
}
