...
import { java, type int, type char, JavaObject } from "jree";
...
this.block = block;
     ...
     */
    private static  parseList(line: java.lang.CharSequence| null, /* final */  markerIndex: int, /* final */  markerColumn: int,
                                      /* final */  inParagraph: boolean):  ListBlockParser.ListData | null {
        let  listMarker: ListBlockParser.ListMarkerData = ListBlockParser.parseListMarker(line, markerIndex);
        if (listMarker === null) {
            return null;
        }
        ...
        if (inParagraph) {
            ...
            if (listBlock instanceof OrderedList && ( listBlock as OrderedList).getMarkerStartNumber() !== 1) {
                return null;
            }
    ...
    private static  parseListMarker(line: java.lang.CharSequence| null, index: int):  ListBlockParser.ListMarkerData | null {
        ...
        switch (c) {
            ...
            case '*':
                if (ListBlockParser.isSpaceTabOrEnd(line, index + 1)) {
                    ...
                    bulletList.setMarker(java.lang.String.valueOf(c));
    ...
    // `)` character.
    private static  parseOrderedList(line: java.lang.CharSequence| null, index: int):  ListBlockParser.ListMarkerData | null {
        let  digits: int = 0;
        let  length: int = line.length();
            ...
            switch (c) {
                ...
                case ')':
                    if (digits >= 1 && ListBlockParser.isSpaceTabOrEnd(line, i + 1)) {
                        let  number: java.lang.String = line.subSequence(index, i).toString();
                        let  orderedList: OrderedList = new  OrderedList();
                        orderedList.setMarkerStartNumber(java.lang.Integer.parseInt(number));
                        orderedList.setMarkerDelimiter(java.lang.String.valueOf(c));
    ...
    private static  isSpaceTabOrEnd(line: java.lang.CharSequence| null, index: int):  boolean {
    ...
    private static  listsMatch(a: ListBlock| null, b: ListBlock| null):  boolean {
        if (a instanceof BulletList && b instanceof BulletList) {
            return java.util.Objects.equals(( a as BulletList).getMarker(), ( b as BulletList).getMarker());
        } else if (a instanceof OrderedList && b instanceof OrderedList) {
            return java.util.Objects.equals(( a as OrderedList).getMarkerDelimiter(), ( b as OrderedList).getMarkerDelimiter());
        }
        return false;
    }
    ...
    public static Factory =  class Factory extends AbstractBlockParserFactory {
        ...
        public  tryStart(state: ParserState| null, matchedBlockParser: MatchedBlockParser| null):  BlockStart | null {
            ...
            let  inParagraph: boolean = !matchedBlockParser.getParagraphLines().isEmpty();
            let  listData: ListBlockParser.ListData = ListBlockParser.parseList(state.getLine().getContent(), markerIndex, markerColumn, inParagraph);
            if (listData === null) {
                return BlockStart.none();
            }

            let  newColumn: int = listData.contentColumn;
            let  listItemParser: ListItemParser = new  ListItemParser(state.getIndent(), newColumn - state.getColumn());
...
export namespace ListBlockParser {
	...
	export type ListMarkerData = InstanceType<typeof ListBlockParser.ListMarkerData>;
}
