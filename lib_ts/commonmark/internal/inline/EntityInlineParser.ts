 ...
 */
export class EntityInlineParser
  extends JavaObject
  implements InlineContentParser
{
  private static readonly hex: AsciiMatcher | null = AsciiMatcher.builder()
    .range("0", "9")
    .range("A", "F")
    .range("a", "f")
    .build();
  private static readonly dec: AsciiMatcher | null = AsciiMatcher.builder()
    .range("0", "9")
    .build();
  private static readonly entityStart: AsciiMatcher | null =
    AsciiMatcher.builder().range("A", "Z").range("a", "z").build();
  private static readonly entityContinue: AsciiMatcher | null =
    EntityInlineParser.entityStart.newBuilder().range("0", "9").build();
