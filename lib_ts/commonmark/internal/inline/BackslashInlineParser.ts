 ...
 */
export class BackslashInlineParser
  extends JavaObject
  implements InlineContentParser
{
  private static readonly ESCAPABLE: java.util.regex.Pattern | null =
    java.util.regex.Pattern.compile("^" + Escaping.ESCAPABLE);
  ...
  public tryParse(
    ...
    if (next === "\n") {
      ...
      BackslashInlineParser.ESCAPABLE.matcher(string.valueOf(next)).matches()
    ) {
      scanner.next();
      return ParsedInline.of(
        new Text(string.valueOf(next)),
        scanner.position()
      );
    } else {
