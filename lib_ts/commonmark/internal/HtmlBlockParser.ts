export class HtmlBlockParser extends AbstractBlockParser {
  ...
  private static readonly ATTRIBUTENAME: string | null =
    "[a-zA-Z_:][a-zA-Z0-9:._-]*";
  private static readonly UNQUOTEDVALUE: string | null =
    "[^\"'=<>`\\x00-\\x20]+";
  private static readonly SINGLEQUOTEDVALUE: string | null = "'[^']*'";
  private static readonly DOUBLEQUOTEDVALUE: string | null = '"[^"]*"';
  private static readonly ATTRIBUTEVALUE: string | null =
    "(?:" +
    ...
    ")";
  private static readonly ATTRIBUTEVALUESPEC: string | null =
    "(?:" + "\\s*=" + "\\s*" + HtmlBlockParser.ATTRIBUTEVALUE + ")";
  private static readonly ATTRIBUTE: string | null =
    "(?:" +
    "\\s+" +
  ...
  private static readonly OPENTAG: string | null =
    "<" + HtmlBlockParser.TAGNAME + HtmlBlockParser.ATTRIBUTE + "*" + "\\s*/?>";
  private static readonly CLOSETAG: string | null =
  ...
  public addLine(line: SourceLine | null): void {
    ...
    if (
      ...
      this.closingPattern.matcher(line.getContent()).find()
    ) {
      this.finished = true;
    }
  }
    ...
    public tryStart(
      ...
      if (state.getIndent() < 4 && line.charAt(nextNonSpace) === "<") {
        for (let blockType: int = 1; blockType <= 7; blockType++) {
          ...
          let opener: java.util.regex.Pattern =
            HtmlBlockParser.BLOCK_PATTERNS[blockType][0];
          let closer: java.util.regex.Pattern =
            HtmlBlockParser.BLOCK_PATTERNS[blockType][1];
          let matches: boolean = opener
            .matcher(line.subSequence(nextNonSpace, line.length()))
            .find();
          if (matches) {
