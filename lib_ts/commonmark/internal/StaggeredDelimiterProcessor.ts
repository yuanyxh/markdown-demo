 ...
 */
export class StaggeredDelimiterProcessor extends JavaObject implements DelimiterProcessor {

    private readonly  delim:  char;
    private  minLength:  int = 0;
    private  processors:  java.util.LinkedList<DelimiterProcessor> | null = new  java.util.LinkedList(); // in reverse getMinLength order
...
this.delim = delim;
    ...
    protected  add(dp: DelimiterProcessor| null): void {
        ...
        let  it: java.util.ListIterator<DelimiterProcessor> = this.processors.listIterator();
        ...
        while (it.hasNext()) {
            ...
            if (len > pLen) {
                ...
                throw new  java.lang.IllegalArgumentException("Cannot add two delimiter processors for char '" + this.delim + "' and minimum length " + len + "; conflicting processors: " + p + ", " + dp);
            }
        }
        if (!added) {
