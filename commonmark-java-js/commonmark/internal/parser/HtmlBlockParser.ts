...
import { HtmlBlock, Paragraph } from '@/node';
...
class HtmlBlockParser extends AbstractBlockParser {
  ...
  private static readonly ATTRIBUTE: string =
    '(?:' + '\\s+' + HtmlBlockParser.ATTRIBUTENAME + HtmlBlockParser.ATTRIBUTEVALUESPEC + '?)';

  private static readonly OPENTAG: string =
    '<' + HtmlBlockParser.TAGNAME + HtmlBlockParser.ATTRIBUTE + '*' + '\\s*/?>';
  private static readonly CLOSETAG: string = '</' + HtmlBlockParser.TAGNAME + '\\s*[>]';

  static readonly BLOCK_PATTERNS = [
    ...
    [
      new RegExp('^(?:' + HtmlBlockParser.OPENTAG + '|' + HtmlBlockParser.CLOSETAG + ')\\s*$', 'i'),
      null // terminated by blank line
    ]
  ];
  ...
  constructor(closingPattern: RegExp | null = null) {
    super();
  ...
  override addLine(line: SourceLine) {
    ...
    if (this.closingPattern !== null && this.closingPattern.exec(line.getContent())) {
      this.finished = true;
    }
  }
